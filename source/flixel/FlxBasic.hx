...
class FlxBasic implements IFlxDestroyable
{
	...
	static public var updatePool:Array<Function> = [];
	static public var drawPool:Array<Function> = [];
	...
	{
		manageUpdatePool(Value);
		return active = Value;
	}

	inline function manageUpdatePool(value:Bool):Void
	{
		if (value)
		{
			if (!updatePool.contains(update))
				updatePool.push(update);
		}
		else
		{
			if (updatePool.contains(update))
				updatePool.remove(update);
		}
	}

	inline function manageDrawPool(value:Bool):Void
	{
		if (value)
		{
			if (!drawPool.contains(draw))
				drawPool.push(draw);
		}
		else
		{
			if (drawPool.contains(draw))
				drawPool.remove(draw);
		}
	}

	inline static function updateBase(elapsed:Float):Void
	{
		for (callback in updatePool)
			callback(elapsed);
	}

	inline static function drawBase():Void
	{
		for (callback in drawPool)
			callback();
	}
